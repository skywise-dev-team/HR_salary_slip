...
export default function UploadSalaryData() {
  ...
  const [needsRegenCount, setNeedsRegenCount] = useState(0);
  ...
  const loadNeedsRegenCount = () => {
    api.get('/api/salary-data/needs-regeneration').then(({ data }) => setNeedsRegenCount(data.length));
  };
  ...
  const regenerate = async (row) => {
    try {
      const { data } = await api.post(`/api/salary-data/${row.id}/regenerate`);
    ...
    <Layout>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-outline" onClick={() => navigate('/upload-salary/regenerate')}>
            Regenerate slips{needsRegenCount > 0 ? ` (${needsRegenCount})` : ''}
          </button>
                        ...
                        {!employees ? (
                          ...
                          <>
                            ...
                            <table className="table" style={{ marginTop: 10 }}>
                              <thead>
                                <tr>
                                  <th>Employee</th><th>Net pay</th><th>Slip status</th><th>Actions</th>
                                </tr>
                              </thead>
                              <tbody>
                                {visibleEmployees.map((e) => (
                                  <tr key={e.id}>
                                    ...
                                    <td>{e.file_exists ? <span className="muted">Present</span> : <span style={{ color: 'var(--danger, #c0392b)' }}>Missing</span>}</td>
                                    <td>
                                      <button className="link-btn" onClick={() => regenerate(e)}>{e.file_exists ? 'Regenerate' : 'Generate'}</button>
