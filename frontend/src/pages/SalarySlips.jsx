...
export default function SalarySlips() {
      ...
      if (!managePerm) {
        ...
        try {
          ...
          if (data?.length) {
            ...
            // month) — not whichever row happens to sort first by
            // generated_at. A bulk import can generate thousands of rows
            // within the same second, so generated_at alone isn't reliable
            // enough to tell which period is actually the latest one.
  ...
  const canManage = can('salary_slips', 'edit');   // full search/filter, delete access elsewhere
  const canDelete = can('salary_slips', 'delete');
  const canDownload = user?.salarySlipAccess === 'VIEW_DOWNLOAD';
  const noAccess = user?.salarySlipAccess === 'NO_ACCESS';
  const isEmployeeLogin = user?.role === 'Employee';
  ...
  const requestRegeneration = async (slip, e) => {
    e?.stopPropagation();
    try {
      const { data } = await api.post(`/api/salary-slips/${slip.id}/request-regeneration`);
  ...
  const remove = async (slip, e) => {
    e?.stopPropagation();
    if (!confirm('Delete this salary slip?')) return;
    await api.delete(`/api/salary-slips/${slip.id}`);
    if (previewSlip?.id === slip.id) setPreviewSlip(null);
    setSelectedIds((ids) => ids.filter((id) => id !== slip.id));
    loadSlips();
  };

  const removeSelected = async () => {
    if (!selectedIds.length) return;
    if (!confirm(`Delete ${selectedIds.length} selected salary slip(s)? This cannot be undone.`)) return;
    try {
      const { data } = await api.delete('/api/salary-slips/bulk', { data: { ids: selectedIds } });
      showToast(data.message || 'Deleted successfully!');
      if (selectedIds.includes(previewSlip?.id)) setPreviewSlip(null);
      setSelectedIds([]);
      loadSlips();
    } catch (err) {
  ...
  return (
    <Layout>
      ...
      {(canDownload || canDelete) && slips.length > 0 && (
        <div style={{ marginBottom: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          {selectedIds.length > 0 && (
            <>
              ...
              {canDelete && (
                <button className="btn btn-outline btn-danger-outline" onClick={removeSelected}>
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>Employee</th><th>Establishment</th><th>Period</th><th>Net pay</th><th>Generated</th>
                {!noAccess && <th>Actions</th>}
              </tr>
            </thead>
            <tbody>
              {slips.flatMap((s) => {
                const missing = isEmployeeLogin && !s.file_exists;
                const rows = [
                  <tr
                    key={s.id}
                    onClick={() => !noAccess && !missing && setPreviewSlip(s)}
                    className={previewSlip?.id === s.id ? 'row-selected' : ''}
                    style={{ cursor: (noAccess || missing) ? 'default' : 'pointer' }}
                  >
                    {(canDownload || canDelete) && (
                    ...
                    {!noAccess && (
                      <td>
                        {missing ? (
                          s.regeneration_requested_at ? (
                            <span className="muted">Requested — awaiting staff</span>
                          ) : (
                            <button className="link-btn" onClick={(e) => requestRegeneration(s, e)}>Request regeneration</button>
                          )
                        ) : (
                          <>
                            ...
                            {canDelete && <button className="link-btn danger" onClick={(e) => remove(s, e)}>Delete</button>}
                          </>
                        )}
                      </td>
                    )}
                ...
                if (previewSlip?.id === s.id) {
                  rows.push(
                    <tr key={`${s.id}-preview`}>
                      <td colSpan={(canDownload || canDelete) ? 7 : 6} style={{ padding: 0 }}>
              ...
              {!slips.length && (
                <tr>
                  <td colSpan={(canDownload || canDelete) ? 7 : 6} className="muted">
