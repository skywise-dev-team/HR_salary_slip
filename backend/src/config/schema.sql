...
-- generated salary slip files (PDF) so "Salary slips" can list / re-download them
CREATE TABLE IF NOT EXISTS salary_slips (
  id              INT AUTO_INCREMENT PRIMARY KEY,
  salary_data_id  INT NOT NULL,
  file_path       VARCHAR(255),
  generated_at    TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  -- Set when an employee requests their slip be regenerated because its PDF
  -- file was removed by the retention cleanup job (or is otherwise missing
  -- on disk) — surfaced to staff on the Upload Salary Data page. Cleared
  -- back to NULL the moment the slip is actually regenerated.
  regeneration_requested_at TIMESTAMP NULL DEFAULT NULL,
  CONSTRAINT fk_slip_data FOREIGN KEY (salary_data_id) REFERENCES salary_data(id) ON DELETE CASCADE,
  UNIQUE KEY uq_slip_salary_data (salary_data_id)
) ENGINE=InnoDB;
