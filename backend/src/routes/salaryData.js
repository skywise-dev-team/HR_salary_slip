...
const LIST_SQL = `
  SELECT sd.id, sd.establishment_id, sd.employee_id, sd.period_month, sd.period_year,
         ...
         ss.id AS slip_id, ss.file_path, ss.regeneration_requested_at
  FROM salary_data sd
  JOIN employees e ON e.id = sd.employee_id
  JOIN establishments es ON es.id = sd.establishment_id
  LEFT JOIN salary_slips ss ON ss.salary_data_id = sd.id`;
...
router.get('/', requirePermission('upload_salary', 'view'), async (req, res) => {
  ...
  // Computed per row, not stored — reflects whether the slip's PDF actually
  // exists on disk right now (it may have been removed by the 12-month
  // retention cleanup, or never generated at all). Powers the per-employee
  // Regenerate/Delete view on the "Regenerate slips" page.
...
// per pay period, with a count of how many salary slips exist for that
...
router.get('/summary', requirePermission('upload_salary', 'view'), async (req, res) => {
  ...
  const [rows] = await pool.query(
    `SELECT es.name AS establishment_name, sd.establishment_id, sd.period_month, sd.period_year,
            COUNT(ss.id) AS slip_count, SUM(sd.net_pay) AS total_net_pay
     FROM salary_data sd
     JOIN establishments es ON es.id = sd.establishment_id
     LEFT JOIN salary_slips ss ON ss.salary_data_id = sd.id
     ${where}
...
// Individual slips whose PDF file is missing on disk — most commonly
// because the 12-month retention cleanup removed it. Flat list (not
// grouped like /summary above) since staff need to act on each one
// individually, and to see which ones an employee has actually asked for.
router.get('/needs-regeneration', requirePermission('upload_salary', 'view'), async (req, res) => {
  const [rows] = await pool.query(
    `SELECT sd.id AS salary_data_id, sd.establishment_id, sd.net_pay, ss.id AS slip_id, ss.file_path, ss.regeneration_requested_at,
            sd.period_month, sd.period_year, e.employee_id AS emp_code, e.full_name, es.name AS establishment_name
     FROM salary_data sd
     JOIN salary_slips ss ON ss.salary_data_id = sd.id
     JOIN employees e ON e.id = sd.employee_id
     JOIN establishments es ON es.id = sd.establishment_id
     ORDER BY sd.period_year DESC, sd.period_month DESC, es.name, e.full_name`
  );
  const missing = rows.filter((r) => !r.file_path || !fs.existsSync(path.join(SLIPS_DIR, r.file_path)));
  res.json(missing);
});

// Regenerates one slip directly (no copy of salary_data is made — this
// just re-renders the PDF from the existing row and writes it back to the
// same deterministic file path, same as the manual regenerate action on
// the Salary Slips page). Clears any pending employee request.
router.post('/:id/regenerate', requirePermission('upload_salary', 'edit'), async (req, res) => {
  try {
    const result = await generateSlipForSalaryData(req.params.id);
    await pool.query('UPDATE salary_slips SET regeneration_requested_at = NULL WHERE id = ?', [result.id]);
    res.json({ message: 'Salary slip regenerated' });
    await logActivity(req, 'UPDATE', 'upload_salary', `Regenerated salary slip for salary data ID ${req.params.id}`);
  } catch (err) {
...
// Deletes every salary_data row (and, via ON DELETE CASCADE, their
// salary_slips rows) for one or more establishment+period groups — i.e.
// entire rows from the Upload Salary Data summary table. The DB cascade
// cleans up the salary_slips rows automatically, but not the PDF files on
// disk, so those are looked up and removed explicitly first.
// Deletes every salary_data row (and, via ON DELETE CASCADE, their
// salary_slips rows) for one or more establishment+period groups — i.e.
// entire rows from the Upload Salary Data summary table. The DB cascade
// cleans up the salary_slips rows automatically, but not the PDF files on
  ...
  const [result] = await pool.query(
    `INSERT INTO salary_data
      (establishment_id, employee_id, period_month, period_year, paid_days, basic, hra, da,
       ...
       tds_deduction, other_deduction, total_deduction, net_pay, uploaded_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [b.establishment_id, b.employee_id, b.period_month, b.period_year, b.paid_days || 0,
     ...
     b.tds_deduction || 0, b.other_deduction || 0, totals.total_deduction, totals.net_pay, uploadedBy]
  );

  let slipWarning = null;
  try { await generateSlipForSalaryData(result.insertId); }
  catch (err) { slipWarning = `Salary record saved, but the slip could not be generated: ${err.message}`; }

  return { id: result.insertId, slipWarning };
}
...
router.post('/', requirePermission('upload_salary', 'edit'), async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: result.id, ...(result.slipWarning ? { warning: result.slipWarning } : {}) });
...
// Applies an edit to an existing salary record directly, keeping its slip
// in sync with the new figures.
async function applyUpdateSalaryData(id, b) {
  ...
  let slipWarning = null;
  try { await generateSlipForSalaryData(id); }
  catch (err) { slipWarning = `Salary record updated, but the slip could not be regenerated: ${err.message}`; }
  return { slipWarning };
}
...
router.put('/:id', requirePermission('upload_salary', 'edit'), async (req, res) => {
  ...
  const result = await applyUpdateSalaryData(req.params.id, req.body);
  res.json({ message: 'Salary record updated', ...(result.slipWarning ? { warning: result.slipWarning } : {}) });
  await logActivity(req, 'UPDATE', 'upload_salary', `Updated salary record ID ${req.params.id}`);
});

// Applies a single salary record deletion directly, cleaning up its slip
// file from disk too.
async function applyDeleteSalaryData(id) {
  const [[slip]] = await pool.query('SELECT file_path FROM salary_slips WHERE salary_data_id = ?', [id]);
  await pool.query('DELETE FROM salary_data WHERE id = ?', [id]);
  if (slip?.file_path) fs.unlink(path.join(SLIPS_DIR, slip.file_path), () => {});
  return {};
}
...
// Shared by the direct-upload path (Admin, streamed, cancellable) and the
// approval-triggered path (everyone else, applied later, no streaming, no
// cancellation possible since there's no live client waiting on it).
// newFilesThisImport tracking (for cancel-rollback file cleanup) is only
// meaningful for the cancellable path — pass null when calling this for an
// approved import, where there's nothing to roll back mid-way.
async function processSalaryDataImportRows(conn, sheet, colIndex, results, reportProgress, isCancelled, newFilesThisImport, uploadedById) {
  for (let r = 2; r <= sheet.rowCount; r++) {
    ...
    try {
      ...
      await conn.query(
        ...
        [emp.establishment_id, emp.id, periodMonth, periodYear, get('paid_days'),
         ...
         gross - totalDed, uploadedById]
      );
      results.inserted++;

      // Slips are generated automatically for every row in the sheet —
      // no separate manual "Generate slip" step needed afterward. Run on
      // the same transaction connection so its salary_slips row rolls
      // back together with the salary_data row if the import is cancelled.
      try {
        const [[savedRow]] = await conn.query(
          'SELECT id FROM salary_data WHERE establishment_id=? AND employee_id=? AND period_month=? AND period_year=?',
          [emp.establishment_id, emp.id, periodMonth, periodYear]
        );
        if (savedRow) {
          // Recognize the slip PDF's target path *before* generating it,
          // so we know whether this run is about to create a brand-new
          // file or overwrite one that already existed. Only files that
          // didn't exist yet are candidates for cleanup on cancel.
          const expectedFilename = `slip_${String(employeeCode).trim()}_${periodMonth}_${periodYear}.pdf`;
          const expectedPath = path.join(SLIPS_DIR, expectedFilename);
          const isNewFile = !fs.existsSync(expectedPath);
          await generateSlipForSalaryData(savedRow.id, conn);
          if (isNewFile && newFilesThisImport) newFilesThisImport.push(expectedPath);
          results.slipsGenerated++;
        }
      } catch (slipErr) {
        results.errors.push(`Row ${r}: salary data saved, but slip generation failed - ${slipErr.message}`);
      }
    } catch (err) {
...
async function applyImportSalaryData(filePath, uploadedById) {
  ...
  const results = { inserted: 0, skipped: 0, slipsGenerated: 0, errors: [] };
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    await processSalaryDataImportRows(conn, sheet, colIndex, results, () => {}, () => false, null, uploadedById);
    await conn.commit();
    return results;
  } catch (err) {
    await conn.rollback();
...
router.post(
  ...
  async (req, res) => {
    ...
    // Everything in this import (including the auto-generated slip rows)
    // runs inside one transaction on a dedicated connection. If the request
    // is aborted mid-way (e.g. the Cancel button on the frontend), the
    // transaction is rolled back in full so no salary_data/salary_slips rows
    // from a cancelled import are left in the database. Any slip PDF that
    // was newly created (not overwriting a pre-existing file) during this
    // same run is also deleted on rollback — files that already existed
    // before the import started are never touched, so a pre-existing slip
    // can't be lost even if its salary_data row happened to be re-imported.
    const conn = await pool.getConnection();
    let cancelled = false;
    let reportProgress, sendDone, sendError;
    const newFilesThisImport = [];
    ...
    try {
      ...
      await processSalaryDataImportRows(conn, sheet, colIndex, results, reportProgress, () => cancelled, newFilesThisImport, req.user.id);

      if (cancelled) {
        await conn.rollback();
        for (const filePath of newFilesThisImport) {
          fs.unlink(filePath, () => {});
        }
      } else {
        await conn.commit();
        sendDone(results);
        await logActivity(req, 'IMPORT', 'upload_salary', `Imported salary data: ${results.inserted} record(s), ${results.slipsGenerated} slip(s) generated, ${results.skipped} skipped`);
      }
    } catch (err) {
