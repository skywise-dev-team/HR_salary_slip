...
// Retention cleanup — runs once a day at 2 AM server time. Removes salary
// slip PDF *files* older than 12 months (by pay period, not upload date;
// the underlying salary_data/salary_slips database rows are never
// touched — only the disk copy) and activity_log entries older than 6
// months. This runs inside the same Node.js process rather than needing a
// separate OS-level scheduled task (Windows Task Scheduler, cron, etc.) —
// as long as the server process itself is running, this fires on schedule
// regardless of platform. Also runs once immediately on startup, so a
// server that's only restarted occasionally (rather than kept running
// continuously) doesn't silently skip cleanup for long stretches.
